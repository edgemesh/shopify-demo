...
The cart bubble is wrapped in a dynamic fragment. The page can use caching while this small region is refreshed for each shopper.
...
“Disable cache” adds `em-bypass=all` to the current URL and performs a full navigation. “Enable cache” removes it and navigates again. Both clear the URL fragment (anchor). Locale, product variant, theme preview, and other query parameters are preserved. `all` bypasses all Edgemesh optimizations, so this compares page loads with Edgemesh optimizations enabled or bypassed, not an isolated edge-cache benchmark. It does not change domain settings or purge shared caches.

While `em-bypass=all` is present, storefront links, GET search forms, and scripted product/search/cart navigation carry it to the next page. Filters preserve it when rebuilding the query string. Destination parameters and anchors remain intact; fragment-only links still scroll without reloading. External links, downloads, account/checkout/admin routes, API endpoints, and cart mutations are excluded. The URL is the only source of this preference: a clean URL uses normal caching, and enabling cache stops propagation. No cookie or browser storage remembers the mode. This requires JavaScript; links are decorated before navigation so no corrective reload is needed.
...
3. On the demo storefront with Edgemesh enabled, verify `<dynamic>` expansion and the `edgemesh-cache` navigation timing entry. Use a second browser session with a different cart to verify isolation.
4. Toggle `em-bypass=all` both directions, including a URL with a variant and preview parameter. Confirm toggle reloads clear anchors and all other parameters survive. While bypassed, navigate home → product → home → cart and open a product in a new tab; confirm the first document request carries bypass. Check search and filtering, then enable cache and confirm propagation stops. Ordinary anchor links should still scroll, and a separately opened clean URL should use normal caching.
5. Do not interpret preview-mode “Not reported” as a cache miss. Verify cache and fragment behavior with Edgemesh enabled before publishing the theme.
